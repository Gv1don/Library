<div class="headTable">
    <h2 class="tableName">Авторы</h2>
    <form action="{{ route('create') }}" method="GET">
        @csrf
        <button name="type" value="authors" type="submit" class="btn btn-primary" style="margin-top: 30px; max-height: 40px; max-width: 200px">+ Добавить автора</button>
    </form>
</div>
<div class="table-responsive">
    <table class="table table-striped table-lg table-hover">
        ...
        <tbody>
            ...
            <tr>
                <td class="text-center author_name" contenteditable="false">{{ $row->name }}</td>
                <td class="col-1">
                    <div class="dashboard">
                        <form action="{{ route('update') }}" method="GET">
                            @csrf
                            <button type="submit" class="btn btn-primary" name="id" value="{{ $row->id }}">
                        ...
                        <form action="{{ route('delete') }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-outline-danger" name="id" value="{{ $row->id }}">
